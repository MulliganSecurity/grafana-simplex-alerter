...
def cmd_string(cmd: ChatCommand) -> str:
    ...
    cmd_builders = {
        ...
        + (" live=on" if c.get("liveMessage") else "")
        + f" json {json.dumps(c['messages'])}",
        # "apiSendMessage": lambda c: f"/_send {c['chatType']}{c['chatId']} json {json.dumps(c['messages'])}",
        "apiUpdateChatItem": lambda c: f"/_update item {c['chatType']}{c['chatId']} {c['chatItemId']}"
        + (" live=on" if c.get("liveMessage") else "")
        + f" json {json.dumps(wrappify(c))}",
        "apiDeleteChatItem": lambda c: f"/_delete item {c['chatType']}{c['chatId']} {c['chatItemId']} {c['deleteMode']}",
        "apiChatRead": lambda c: f"/_read chat {c['chatType']}{c['chatId']}"
        + (
            f" from={c['itemRange']['fromItem']} to={c['itemRange']['toItem']}"
            if c.get("itemRange")
            else ""
        ),
        "apiChatItemsRead": lambda c: f"/_read chat items {c['chatType']}{c['chatId']} "
        + (
            str(c["msgIds"])
            if isinstance(c["msgIds"], int)
            else " ".join(str(i) for i in c["msgIds"])
        ),
