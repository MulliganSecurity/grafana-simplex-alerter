...
class ChatClient:
    ...
    @classmethod
    async def create(
        cls, server: Union[ChatServer, str] = None, cfg: ChatClientConfig = None
    ) -> "ChatClient":
        ...
        transport = await ChatTransport.connect(
            server, cfg.tcp_timeout / 1000, cfg.q_size
        )
    ...
    async def api_get_active_user(self) -> Optional[User]:
        ...
        if r["type"] == "activeUser":
            ...
            if (
                r["chatError"]["type"] == "error"
                and r["chatError"]["errorType"]["type"] == "noActiveUser"
            ):
                return None
    ...
    async def api_create_active_user(
        self,
    ...
    async def api_set_incognito(self, incognito: bool) -> None:
        ...
        return await self.ok_chat_command(
            {"type": "setIncognito", "incognito": incognito}
        )

    async def enable_address_auto_accept(
        self, accept_incognito: bool = False, auto_reply: Optional[MsgContent] = None
    ) -> None:
    ...
    async def api_send_text_message(
        ...
        if live:
            ...
            message = {"msgContent": {"type": "text", "text": text}}
        return await self.api_send_messages(chat_type, chat_id, [message], is_live=live)
    ...
    async def api_delete_chat_item(
        self,
        chat_type: ChatType,
        chat_id: int,
        chat_item_id: int,
    ...
    async def api_delete_chat(self, chat_type: ChatType, chat_id: int) -> None:
        """Delete a chat."""
        r = await self.send_chat_command(
            {"type": "apiDeleteChat", "chatType": chat_type.value, "chatId": chat_id}
        )
        ...
        if chat_type == ChatType.Direct and r["type"] == "contactDeleted":
            ...
            and r["type"] == "contactConnectionDeleted"
        ):
            return
    ...
    async def api_clear_chat(self, chat_type: ChatType, chat_id: int) -> ChatInfo:
        ...
        r = await self.send_chat_command(
            {"type": "apiClearChat", "chatType": chat_type.value, "chatId": chat_id}
        )
    ...
    async def api_update_profile(
        self, user_id: int, profile: Profile
    ) -> Optional[Profile]:
        """Update a user profile."""
        r = await self.send_chat_command(
            {"type": "apiUpdateProfile", "userId": user_id, "profile": profile}
        )
    ...
    async def api_get_user_address(self) -> Optional[str]:
        ...
        if r["type"] == "userContactLink":
            ...
            r["type"] == "chatCmdError"
            and r["chatError"]["type"] == "errorStore"
            and r["chatError"]["storeError"]["type"] == "userContactLinkNotFound"
        ):
            return None
        else:
    ...
    async def api_accept_contact_request(self, contact_req_id: int) -> Contact:
        ...
        r = await self.send_chat_command(
            {"type": "apiAcceptContact", "contactReqId": contact_req_id}
        )
    ...
    async def api_reject_contact_request(self, contact_req_id: int) -> None:
        ...
        r = await self.send_chat_command(
            {"type": "apiRejectContact", "contactReqId": contact_req_id}
        )
    ...
    async def api_chat_read(
        self, chat_type: ChatType, chat_id: int, ids: Union[int, List[int]]
    ) -> None:
    ...
    async def api_contact_info(
        self, contact_id: int
    ) -> Tuple[Optional[ConnectionStats], Optional[Profile]]:
        """Get information about a contact."""
        r = await self.send_chat_command(
            {"type": "apiContactInfo", "contactId": contact_id}
        )
    ...
    async def api_group_member_info(
    ...
    async def api_receive_file(self, file_id: int) -> AChatItem:
        ...
        r = await self.send_chat_command({"type": "receiveFile", "fileId": file_id})
    ...
    async def api_add_member(
        self, group_id: int, contact_id: int, member_role: GroupMemberRole
    ) -> GroupMember:
    ...
    async def api_join_group(self, group_id: int) -> GroupInfo:
        """Join a group."""
        r = await self.send_chat_command({"type": "apiJoinGroup", "groupId": group_id})
    ...
    async def api_leave_group(self, group_id: int) -> GroupInfo:
        """Leave a group."""
        r = await self.send_chat_command({"type": "apiLeaveGroup", "groupId": group_id})
    ...
    async def api_list_members(self, group_id: int) -> List[GroupMember]:
        ...
        r = await self.send_chat_command(
            {"type": "apiListMembers", "groupId": group_id}
        )
    ...
    async def api_update_group(
        self, group_id: int, group_profile: GroupProfile
    ) -> GroupInfo:
