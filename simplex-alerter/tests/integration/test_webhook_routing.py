...
def reset_webhook_globals():
    ...
    original_endpoint_group_map = webhook_module.endpoint_group_map.copy()
    original_simplex_endpoint = webhook_module.simplex_endpoint
    yield
    webhook_module.endpoint_group_map.clear()
    webhook_module.endpoint_group_map.update(original_endpoint_group_map)
    webhook_module.simplex_endpoint = original_simplex_endpoint
...
def test_client(mock_client):
    """Build a TestClient with startup bypassed."""
    webhook_module.simplex_endpoint = "ws://localhost:7897"

    with patch("simplex_alerter.webhook.ChatClient.create", return_value=mock_client), \
         ...
         patch("simplex_alerter.webhook.subprocess.Popen"), \
         ...
         patch("simplex_alerter.webhook.initialize_telemetry", new=AsyncMock()):
...
def test_post_to_known_group_returns_200(test_client):
    ...
    with patch("simplex_alerter.webhook.ChatClient.create", return_value=mock_chat), \
         patch("simplex_alerter.webhook.get_groups", new=AsyncMock(return_value={"mygroup": 42})):
